...
var isUndefined = require('../../_chunks/dep-0b181efe.js');
var cloneDeep = require('../../_chunks/dep-455c76a3.js');
var isEqual = require('../../_chunks/dep-2a185a5d.js');
...
function valueInArray(val, arr) {
  return arr.some(function (item) {
    return isEqual.isEqual(item, val);
  });
}
...
function getDataValues(data, filterValues) {
  ...
  if (isTreeMode) {
    ...
    if (data) {
      for (var i = 0; i < data.length; i++) {
        var item = data[i];
        var isInclude = valueInArray(item.value, filterValues) && !item.disabled;
  ...
  return data.filter(function (item) {
    ...
    var isInclude = valueInArray(item.value, filterValues);
    return (include && isInclude || !include && !isInclude) && (!item.disabled || valueInArray(item.value, remainValue));
  }).map(function (item) {
    return item.value;
  });
}
...
function filterTransferData(data, filterValues) {
  ...
  if (!isTreeMode) {
    if (needMatch) {
      return filterValues === null || filterValues === void 0 ? void 0 : filterValues.map(function (value) {
        return data.find(function (item) {
          return isEqual.isEqual(item.value, value);
        });
    ...
    return data.filter(function (item) {
      var isMatch = valueInArray(item.value, filterValues);
      return !isMatch;
    });
  }
  var result = [];
...
exports.isTreeNodeValid = isTreeNodeValid;
