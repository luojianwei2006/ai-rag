...
- **钉钉通知**: 商户配置 Webhook + 加签密钥，客户消息触发推送（同一会话 15 分钟内不重复推送，超过 15 分钟有新消息即推送，含嵌入监控链接）
- **FAQ 管理**: 商户可配置分类/问题/答案，客户聊天前先展示 FAQ，点击「联系客服」后进入聊天

## 钉钉通知推送

商户在后台「设置」页配置「钉钉 Webhook 地址」和「加签密钥(secret)」后，客户发来消息时自动向钉钉群推送通知，方便客服及时响应。

### 触发规则（纯时间节流）

- 会话**首条**客户消息：立即推送
- 同一会话 **15 分钟内**的后续任意客户消息：**不重复推送**（连发多条也会被 15 分钟窗口挡住）
- 同一会话**超过 15 分钟**再有新消息（无论是否是老客户复用会话、是否连续消息）：**再次推送**
- 阈值边界：距上次推送**满 15 分钟整**即允许再次推送（不含第 15 分钟内的消息）

### 推送内容

纯文本消息，包含：商户名称、客户 UID、客户昵称、消息内容，以及一键进入「嵌入监控」的链接（含 `api_key`，可直接查看并回复）。

### 健壮性

- Webhook 或密钥未配置时不推送
- 推送失败（密钥错误、机器人被移出群等）仅记录日志，**不影响客户正常聊天**
- 推送时间戳在发送前同步落库，避免 15 分钟窗口内重复尝试
...
- `00c38176` 钉钉推送改为**纯时间节流**：同一会话 15 分钟内不重复推送，超过 15 分钟有新消息即推送（无论是否新轮次 / 老客户复用会话 / 连续消息）。为当前生效规则。
- `07342c3e` 钉钉推送增加 **15 分钟节流窗口**（在「新轮次」触发之上叠加时间节流，复用 `ChatSession.last_dingtalk_notify` 字段记录时间戳）。
- `498b5716` 钉钉推送触发从「仅会话首条」扩展为「**新会话首条 或 客户在新一轮开口**」即推送，覆盖老客户复用会话与同会话追问两个漏推场景。
